    ...
    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden sticky top-24">
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
          <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase mb-4 tracking-widest">Digital Ledger</p>
          <div className="space-y-3">
             <div className="flex justify-between items-center text-xs font-medium">
               <span className="text-slate-500 dark:text-slate-400">Account Opening</span>
               <span className="text-green-600 dark:text-green-400">+$500.00</span>
             </div>
             {wallet.escrow > 0 && (
               <div className="flex justify-between items-center text-xs font-bold">
                 <span className="text-slate-700 dark:text-slate-300">Active Holds</span>
                 <span className="text-amber-600 dark:text-amber-400">-${wallet.escrow.toFixed(2)}</span>
               </div>
             )}
          </div>
        </div>
